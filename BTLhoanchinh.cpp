...
void findVehicleApprox(Stack &parkingXM, Stack &parkingOTO,
     Queue &waitingXM, Queue &waitingOTO) {
    int choice;
    string key;
    bool foundAny = false;

    cout << "\n=== CHỌN KIỂU TÌM KIẾM ===\n";
    cout << "1. Tìm gần đúng theo MÃ SỐ VÉ\n";
    cout << "2. Tìm gần đúng theo BIỂN SỐ XE\n";
    ...
    cout << "\n┌─────────────────────── KẾT QUẢ TÌM KIẾM ───────────────────────────────────┐\n";
    cout << "│ " << left << setw(4) << "ID" << " │ " << setw(15) << "BIỂN SỐ" << "    |       "
         << setw(10) << "LOẠI" << " │ " << setw(15) << "VỊ TRÍ" << " |  "
         << setw(25) << "GIỜ VÀO" << "   \n";
    cout << "├────────────────────────────────────────────────────────────────────────────┤\n";

    Vehicle* lists[4] = { parkingXM.top, parkingOTO.top, waitingXM.head, waitingOTO.head };
    string names[4] = { "Bãi XM", "Bãi OTO", "Hàng chờ XM", "Hàng chờ OTO" };
    
 if (choice == 2) key = toUpperStr(key);
    for (int i = 0; i < 4; i++) {
        Vehicle* cur = lists[i];
        while (cur != NULL) {
            bool match = false;
            ...
            if (choice == 1) {
                string tmp = to_string(cur->info.id);
                // Nếu chuỗi nhập có ký tự trùng trong idStr
                for (int j = 0; j < key.size(); j++) {
                    for (int k = 0; k < tmp.size(); k++) {
                        if (key[j] == tmp[k]) match = true;
                    }
                }
            }
            else if (choice == 2) {
                string plateU = toUpperStr(cur->info.plate);
                // Nếu chuỗi nhập có ký tự trùng trong biển số
                for (int j = 0; j < key.size(); j++) {
                    for (int k = 0; k < plateU.size(); k++) {
                        if (key[j] == plateU[k]) match = true;
                    }
                }
            }
    ...
    if (!foundAny) {
        cout << ">> Không tìm thấy xe có ký tự gần đúng với: " << key << "\n";
    }
}
// chức năng 1 
...
// chức năng 11
// ======= Chức năng 9: Tìm và hiển thị xe theo mã vé hoặc biển số =======
void quanLyTimXe(Stack &baiXeXeMay, Stack &baiXeOto, Queue &hangChoXeMay, Queue &hangChoOto, LichSuManager &lichSu) {
    int luaChon;
    cout << "\nChọn kiểu tìm kiếm:\n";
    cout << "1. Theo mã vé\n";
    cout << "2. Theo biển số (chính xác)\n";
    cout << "Nhập lựa chọn: ";
    ...
    if (luaChon == 1) {
        ...
        for (int i = 0; i < 4 && !timThay; i++) {
            hienTai = danhSach[i];
            while (hienTai) {
                if (toUpperStr(hienTai->info.plate) == bienSoChuan) {
                    timThay = true;
                    break;
                }
                hienTai = hienTai->next;
    ...
    if (!timThay) {
        if (luaChon == 1) 
            cout << ">> Không tìm thấy mã vé. Xem lịch sử xe đã ra:\n";
        else 
            cout << ">> Không tìm thấy xe theo biển số. Xem lịch sử xe đã ra:\n";
        
        lichSu.showAllHistory();  // Gọi đúng đối tượng
        return;
    }
    ...
    cout << "|                           CHI TIẾT XE                                    |\n";
    cout << "+==========================================================================+\n";
    cout << "| Mã vé          | " << setw(53) << left << hienTai->info.id << "\n";
    cout << "| Biển số        | " << setw(53) << left << hienTai->info.plate << "\n";
    cout << "| Loại xe        | " << setw(53) << left << hienTai->info.type << "\n";
    cout << "| Giờ vào        | " << setw(53) << left << formatTime(hienTai->info.inTime) << "\n";
    cout << "| Giờ ra         | " << setw(53) << left << "-" << "\n";
    cout << "| Thời gian gửi  | " << setw(53) << left << "-" << "\n";
